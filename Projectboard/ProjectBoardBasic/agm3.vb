...
Public Module agm3
    ...
    Public Function checkOrgaImportConfig(ByVal configFile As String,
        ...
        Dim configName As String = "configuration Orga"
        Dim configSettLine As New clsConfigLine
        ...
        Dim configuration As New List(Of clsConfigLine)
        configuration = CType(databaseAcc, DBAccLayer.Request).retrieveConfigurationsFromDB(configName, Date.Now, False, err)
        ...
        If Not IsNothing(configuration) Then
            ...
            Try
                For i = 0 To lastrow - 1
                    ...
                    configSettLine = configuration.ElementAt(i)
                    configLine = New clsConfigOrgaImport

                    Dim Titel As String = configSettLine.Titel

                    Select Case Titel
                        Case "DateiName"
                            configLine.Titel = configSettLine.Titel
                            configLine.Inputfile = configSettLine.InputFile
                            'orgaImportConfigs.Add(Titel, configLine)

                        Case Else
                            configLine.Titel = configSettLine.Titel
                            configLine.Identifier = configSettLine.Identifier
                            configLine.Inputfile = configSettLine.InputFile
                            configLine.Typ = configSettLine.Typ
                            configLine.cellrange = (CStr(configSettLine.Datenbereich) = "Range")

                            ' check out, whether there really is a valid Range Definition 
                            Dim validRange As Boolean = True

                            If configLine.cellrange Then
                                Dim tabrange As String = configSettLine.TabellenNummer
                                Dim hstr() As String = Split(tabrange, ":")
                                If hstr.Length = 2 Then
                                    configLine.column.von = CInt(hstr(0))
                                    configLine.column.bis = CInt(hstr(1))
                                ElseIf hstr.Length = 1 Then
                                    If IsNumeric(configSettLine.TabellenNummer) Then
                                        configLine.column.von = CInt(configSettLine.TabellenNummer)
                                        configLine.column.bis = CInt(configSettLine.TabellenNummer)
                                    Else
                                        configLine.column.von = 0
                                        configLine.column.bis = 0
                                    End If
                                Else
                                    outputLine = configLine.Titel & " : Angabe ist kein Range"
                                    validRange = False
                            ...
                            configLine.sheetDescript = configSettLine.TabellenName

                            ' check out, whether there really is a valid Range Definition 
                            validRange = True

                            If configLine.cellrange Then
                                Dim colrange As String = configSettLine.SpaltenNummer
                                Dim hstr() As String = Split(colrange, ":")
                                If hstr.Length = 2 Then
                                    configLine.column.von = CInt(hstr(0))
                                    configLine.column.bis = CInt(hstr(1))
                                ElseIf hstr.Length = 1 Then
                                    If IsNumeric(configSettLine.SpaltenNummer) Then
                                        configLine.column.von = CInt(configSettLine.SpaltenNummer)
                                        configLine.column.bis = CInt(configSettLine.SpaltenNummer)
                                    Else
                                        configLine.column.von = 0
                                        configLine.column.bis = 0
                                    End If
                            ...
                            ' now consider potential Row Range 
                            If configLine.cellrange Then
                                Dim rowrange As String = configSettLine.ZeilenNummer
                                Dim hstr() As String = Split(rowrange, ":")
                                If hstr.Length = 2 Then
                                    configLine.row.von = CInt(hstr(0))
                                    configLine.row.bis = CInt(hstr(1))
                                ElseIf hstr.Length = 1 Then
                                    If IsNumeric(configSettLine.SpaltenNummer) Then
                                        configLine.row.von = CInt(configSettLine.ZeilenNummer)
                                        configLine.row.bis = CInt(configSettLine.ZeilenNummer)
                                    Else
                                        configLine.row.von = 0
                                        configLine.row.bis = 0
                                    End If
                                Else
                                    validRange = False
                                    If Not validRange Then
                                ...
                                End If
                            Else
                                If IsNumeric(configSettLine.ZeilenNummer) Then
                                    configLine.row.von = CInt(configSettLine.ZeilenNummer)
                                    configLine.row.bis = CInt(configSettLine.ZeilenNummer)
                                Else
                                    configLine.row.von = 0
                            ...
                            configLine.columnDescript = CStr(configSettLine.Spaltenüberschrift)
                            configLine.rowDescript = CStr(configSettLine.Zeilenbeschriftung)
                            configLine.objType = CStr(configSettLine.ObjektTyp)
                            configLine.content = CStr(configSettLine.Inhalt)
                    End Select
            ...
            Catch ex As Exception
                ...
                If awinSettings.englishLanguage Then
                    Call MsgBox("The configuration-Orga Setting '" & configName & "'  used to import the Orga downloaded from WebUI couldn't be read.")
                    outputLine = "The configurationfile '" & configName & "'  used to import the Orga downloaded from WebUI couldn't be read."
                Else
                    Call MsgBox("Das Lesen des KonfigurationSetting '" & configName & "' war nicht erfolgreich." &
                                vbCrLf & " Die Organisation kann somit nicht importiert werden")
                    outputLine = "Das Lesen der KonfigurationSetting '" & configName & "' war nicht erfolgreich." &
                                vbCrLf & " Die Organisation kann somit nicht importiert werden"
                End If
                outputCollection.Add(outputLine)
            End Try
        ...
        Else
            If awinSettings.englishLanguage Then
                outputLine = "The configuration-Orag Setting doesn't exist!  -  " & configName
            Else
                outputLine = "Die Konfiguration-Orga Setting existiert nicht!  -  " & configName
            End If
            outputCollection.Add(outputLine)
        End If
        ...
        '                TypCol = searcharea.Find("Typ").Column
        '                DatenCol = searcharea.Find("Datenbereich").Column
        '                TabUCol = searcharea.Find("TabellenName").Column
        '                TabNCol = searcharea.Find("TabellenNummer").Column
        '                SUCol = searcharea.Find("Spaltenüberschrift").Column
        '                SNCol = searcharea.Find("SpaltenNummer").Column
        '                ZUCol = searcharea.Find("Zeilenbeschriftung").Column
        '                ZNCol = searcharea.Find("ZeilenNummer").Column
        '                ObjCol = searcharea.Find("ObjektTyp").Column
        '                InhaltCol = searcharea.Find("Inhalt").Column
    ...
    Public Function checkJIRAProjectImportConfig(ByVal configFile As String,
                                      ByRef ProjectsFile As String,
                                      ByRef ProjectsConfigs As SortedList(Of String, clsConfigProjectsImport),
                                      ByRef lastrow As Integer,
                                      ByRef outputCollection As Collection) As Boolean

        Dim configName As String = "configuration JIRA"
        Dim configSettLine As New clsConfigLine
        Dim configLine As New clsConfigProjectsImport
        ...
        If Not IsNothing(configuration) And configuration.count > 0 Then
            ...
            Try
                For i = 0 To lastrow - 1
                    ...
                    Select Case Titel
                        Case "DateiName"
                            configLine.Titel = configSettLine.Titel
                            configLine.ProjectsFile = configSettLine.InputFile
                            ProjectsFile = configSettLine.InputFile
                        ...
                        Case Else
                            ...
                            configLine.cellrange = (CStr(configSettLine.Datenbereich) = "Range")
                            If IsNumeric(configSettLine.TabellenNummer) Then
                                configLine.sheet = CInt(configSettLine.TabellenNummer)
                            Else
                                configLine.sheet = 0
                            End If
                            configLine.sheetDescript = configSettLine.TabellenName
                            ...
                            ' now consider potential Row Range 
                            If configLine.cellrange Then
                                Dim colrange As String = configSettLine.ZeilenNummer
                                Dim hstr() As String = Split(colrange, ":")
                                If hstr.Length = 2 Then
                                    configLine.row.von = CInt(hstr(0))
                                    configLine.row.bis = CInt(hstr(1))
            ...
            Catch ex As Exception
                If awinSettings.englishLanguage Then
                    Call MsgBox("The configuration-JIRA Setting '" & configName & "'  used to import the projects couldn't be read.")
                    outputLine = "The configurationfile '" & configName & "'  used to import the projects couldn't be read."
                Else
                    Call MsgBox("Das Lesen des KonfigurationSetting '" & configName & "' war nicht erfolgreich." &
                                vbCrLf & " Die Projekte können somit nicht importiert werden")
                    outputLine = "Das Lesen der KonfigurationSetting '" & configName & "' war nicht erfolgreich." &
        ...
        checkJIRAProjectImportConfig = (ProjectsConfigs.Count > 0) And (outputCollection.Count = oldAnzMsg)
    ...
    Public Function readActualDataWithConfig(ByVal ActualDataConfig As SortedList(Of String, clsConfigActualDataImport),
        ...
        Try
            If My.Computer.FileSystem.FileExists(tmpDatei) Then
                                            ...
                                            If Not IsNothing(projektKDNr) Then
                                                ...
                                                If projKDNrConfig.objType = "RegEx" Then
                                                    If Not IsNothing(projKDNrConfig.content) Then
                                                        ...
                                                        Else
                                                            ' 29.3.25 adjust for Telair 
                                                            Dim teile As String() = projektKDNr.Split("#"c)
                                                            If teile.Length > 1 Then
                                                                projektKDNr = teile(1) & "-" & teile(0)
                                                            Else
                                                                ' ProjektKDNr remains unchanged
                                                            End If
                                                ...
                                                If projektKDNr <> "" Then
                                                    ...
                                                    Else
                                                        ...
                                                        If pNames.Count = 1 Then
                                                            ...
                                                            ' this will produce lines for each project because of the changed names convention, tk 30.3.25
                                                            ' that is why it is taken out 
                                                        ...
                                                        ElseIf pNames.Count > 1 Then
                                                            ...
                                                            Dim allNames As String = pNames.ToString
                                                            If awinSettings.englishLanguage Then
                                                                outputline = "There exists more than one project to project No. '" & projektKDNr & " : " & allNames & "'"
                                                            Else
                                                                outputline = "Zu Projekt-Nr. '" & projektKDNr & "'" & " existieren mehrer Projekte" & " : " & allNames
                                                            End If
                                                    ...
                                                    Else
                                                        ...
                                                        ' 30.3.25 now check whether or not the Actual Data is 
                                                        Try
                                                        ...
                                                        Catch ex As Exception
                                                            ...
                                                            If awinSettings.englishLanguage Then
                                                                outputline = "In File: " & tmpDatei & vbLf & "project " & hproj.name & " does not cover this month: " & currentWS.Name
                                                            Else
                                                                outputline = "In Datei: " & tmpDatei & vbLf & "Projekt " & hproj.name & " deckt diesen Monat nicht ab: " & currentWS.Name
                                                            End If
                ...
                Catch ex As Exception
                    actDataWB = Nothing
                    outputline = tmpDatei & " : " & personalName & " : " & pName
                    oPCollection.Add(outputline)
                    Call logger(ptErrLevel.logError, "1. " & outputline & vbLf & ex.Message, "readActualDataWithConfig", anzFehler)
                    Call MsgBox("1. " & outputline & vbLf & ex.Message)
                End Try
        ...
        Catch ex As Exception
            outputline = tmpDatei & " : " & personalName & " : " & pName
            oPCollection.Add(outputline)
            Call logger(ptErrLevel.logError, "2. " & outputline & vbLf & ex.Message, "readActualDataWithConfig", anzFehler)
            Call MsgBox("2. " & outputline & vbLf & ex.Message)
        End Try
    ...
    ''' <summary>
    ''' imports capacities from an Zeuss Export file, no fancy monthly overlaps, just day 1 until last day of that month ...
    ''' </summary>
    ''' <param name="kapaConfig">Name of Configuration-File </param>
    ''' <param name="kapaFileName">Name of Zeuss (Std) files which shall be read</param>
    ''' <param name="oPCollection">collection showing all error messages</param>
    ''' <returns></returns>
    Friend Function readZeussStdWithConfig(ByVal kapaConfig As SortedList(Of String, clsConfigKapaImport),
        ...
        ' used to hold information about columnOfMonth_Considered and which columns in the excel File belong to that month
        Dim listofColumns As New SortedList(Of Integer, KeyValuePair(Of Integer, Integer))
            ...
            Try
                For index = 1 To appInstance.Worksheets.Count
                    ...
                    Else
                        ...
                        ' now define ListOfColumns
                        Dim dateRow As Integer = kapaConfig("dateRow").row
                        Dim dateRowLastSpalte As Integer = CType(currentWS.Cells(dateRow, 20000), Global.Microsoft.Office.Interop.Excel.Range).End(Excel.XlDirection.xlToLeft).Column

                        ' now position to the first month beginning with 1
                        Dim si As Integer = 0
                        Dim startedYet As Boolean = False

                        Dim tmpLastMonthCol As Integer = -1
                        Dim tmpStartCol As Integer

                        Do While si <= dateRowLastSpalte
                            si = si + 1
                            If Not IsNothing(CType(currentWS.Cells(dateRow, si), Global.Microsoft.Office.Interop.Excel.Range).Value) Then
                                Try
                                    Dim tmpDate = CDate(CType(currentWS.Cells(dateRow, si), Global.Microsoft.Office.Interop.Excel.Range).Value)
                                    If Not IsNothing(tmpDate) Then
                                        If Not IsNothing(CType(currentWS.Cells(dateRow + 1, si), Global.Microsoft.Office.Interop.Excel.Range).Value) Then
                                            If CInt(CType(currentWS.Cells(dateRow + 1, si), Global.Microsoft.Office.Interop.Excel.Range).Value) = 1 Then
                                                If Not startedYet Then
                                                    tmpLastMonthCol = getColumnOfDate(tmpDate)
                                                    tmpStartCol = si
                                                Else
                                                    Dim newRange As New KeyValuePair(Of Integer, Integer)(tmpStartCol, si - 1)
                                                    listofColumns.Add(getColumnOfDate(tmpDate), newRange)
                                                    tmpStartCol = si
                                                End If
                                            End If
                                        Else
                                            ' Error: month does not begin with 1
                                            ' should eventually cause exit  
                                            ' not existing employeeNr ..) 
                                            msgtxt = "Error when reading date Row: Month " & tmpDate.ToString & " does not start with day 1 in row below"
                                            Call logger(ptErrLevel.logError, msgtxt, kapaFileName, anzFehler)
                                            oPCollection.Add(msgtxt, msgtxt)
                                        End If
                                ...
                                Catch ex As Exception
                                    msgtxt = "Error when reading Row " & dateRow & ex.Message
                                    Call logger(ptErrLevel.logError, msgtxt, kapaFileName, anzFehler)
                                    oPCollection.Add(msgtxt, msgtxt)
                                End Try
                        ...
                        ' tk - das wird doch eigentlich gar nicht mehr gebraucht ... lastLine ist einfach letzte Zeile, die content enthät  
                        ...
                        For iZ = firstUrlzeile To lastZeile
                            ...
                            Try
                                employeeNr = CStr(CType(currentWS.Cells(iZ, kapaConfig("employeeNr").column), Global.Microsoft.Office.Interop.Excel.Range).Value).Trim
                            Catch ex As Exception
                                employeeNr = ""
                            End Try

                            hrole = Nothing
                            ' try to define the according role ..
                            ' prio 1: try to find it via roleUID 

                            If employeeNr <> "" Then
                                hrole = RoleDefinitions.getRoledefByEmployeeNr(employeeNr)
                                If IsNothing(hrole) Then
                                    ' not existing employeeNr ..) 
                                    msgtxt = "EmployeeNr does not exist in VISBO Organisation: Zeile " & iZ & "; " & employeeNr
                                    Call logger(ptErrLevel.logInfo, msgtxt, kapaFileName, anzFehler)
                                End If
                            End If

                            If IsNothing(hrole) Then
                                ' try to find it via name
                                If rolename <> "" Then
                                    hrole = RoleDefinitions.getRoledef(rolename)
                                End If
                            ElseIf rolename <> "" Then
                                ' check whether or not UID does reference the same Name - if not, then continue but write log message
                                Dim hRoleN As clsRollenDefinition = RoleDefinitions.getRoledef(rolename)
                                If hRoleN.name = hrole.name Then
                                    ' everything ok
                                Else
                                    ' log mismatch, but continue with employeeNr identified role
                                    msgtxt = "MisMatch EmployeeNr and Name in Line: " & iZ & "; " & employeeNr & " (-> " & hrole.name & "), " & rolename
                                    Call logger(ptErrLevel.logError, msgtxt, kapaFileName, anzFehler)
                                    oPCollection.Add(msgtxt, msgtxt)
                                End If
                            ...
                            If Not IsNothing(hrole) Then
                                ...
                                ' now determine the ranges which need to be read
                                Dim defaultHrsPerdayForThisPerson As Double = hrole.defaultDayCapa

                                For Each item As KeyValuePair(Of Integer, KeyValuePair(Of Integer, Integer)) In listofColumns

                                    Dim anzArbTage As Double = 0
                                    Dim anzArbStd As Double = 0

                                    For sp = item.Value.Key To item.Value.Value

                                        If sp <= lastSpalte Then
                                    ...
                                    If item.Key >= getColumnOfDate(hrole.entryDate) And item.Key < getColumnOfDate(hrole.exitDate) Then
                                        hrole.kapazitaet(item.Key) = anzArbTage
                                    Else
                                        hrole.kapazitaet(item.Key) = 0
                                    End If
        ...
        result = (oPCollection.Count = old_oPCollectionCount)


        readZeussStdWithConfig = result
    End Function
    ...
    Public Function readCalcTemplatesWithConfig(ByVal projectConfig As SortedList(Of String, clsConfigProjectsImport),
        ...
        Dim projektleiter As String = ""
        Dim projDescription As String = ""
        ...
        Try
            If My.Computer.FileSystem.FileExists(tmpDatei) Then
                Try
                    ...
                    'added 9.3.2022
                    Dim projektleiterDefinition As clsConfigProjectsImport = projectConfig("Projektleiter")
                    Dim phaseDefinition As clsConfigProjectsImport = projectConfig("Phase")
                    Dim orgaUnitDefinition As clsConfigProjectsImport = projectConfig("OrgaUnit")
                    Dim skillDefinition As clsConfigProjectsImport = projectConfig("Skill")
                    Dim projDescDefinition As clsConfigProjectsImport = projectConfig("Projekt-Ziel")
                    ...
                    If Not isValidProjectName(pName) Then
                        Dim oldPname As String = pName
                        pName = makeValidProjectName(oldPname)
                        outputline = "Project-Name changed from " & oldPname & " to " & pName
                        Call logger(ptErrLevel.logInfo, outputline, "readCalcTemplatesWithConfig", anzFehler)
                    End If
                    ...
                    ' read Projekt-Ziele 
                    searchColNr = projDescDefinition.column.von
                    If searchColNr > 0 Then
                        Try
                            searchColumn = currentWS.Columns(searchColNr)
                            myRowNr = searchColumn.Find(What:=projDescDefinition.Identifier).Row
                            offsets = projDescDefinition.getRowColumnOffset
                            If Not IsNothing(currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1)).value) Then
                                projDescription = CStr(currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1)).value).Trim
                            Else
                                projDescription = ""
                            End If
                        Catch ex As Exception
                    ...
                    ' read Kunde
                    If kundeDefinition.column.von > 0 Then
                        Try
                    ...
                    If kunde <> "" And projDescription = "" Then
                        ' used especially for external customer projects
                        projDescription = "Project for " & kunde
                    End If


                    ' read Projektleiter 
                    If projektleiterDefinition.column.von > 0 Then

                        Try
                            searchColNr = projektleiterDefinition.column.von
                            searchColumn = currentWS.Columns(searchColNr)

                            myRowNr = searchColumn.Find(What:=projektleiterDefinition.Identifier).Row
                            offsets = projektleiterDefinition.getRowColumnOffset

                            If Not IsNothing(currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1)).value) Then
                                projektleiter = CStr(currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1)).value).Trim
                            Else
                                projektleiter = ""
                            End If
                        Catch ex As Exception
                    ...
                    ' read bu
                    If buDefinition.column.von > 0 Then
                    ...
                    ' read budget / erloes
                    If budgetDefinition.column.von > 0 Then
                        ...
                        Try
                            ...
                            If Not IsNothing(currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1)).value) Then
                                ...
                                Try
                                    Dim cell As Excel.Range = currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1))
                                    Dim CellFormat As String = cell.DisplayFormat.NumberFormatLocal.ToString
                                    If CellFormat.EndsWith("€") And Not CellFormat.EndsWith("T€") Then
                                        faktor = 1000
                                    End If
                                Catch ex As Exception
                                ...
                                budget = CDbl(currentWS.Cells(myRowNr + offsets(0), searchColNr + offsets(1)).value) / faktor
                            Else
                                budget = 0
                            End If
                        Catch ex As Exception
                    ...
                    Dim existingProj As clsProjekt = getProjektFromSessionOrDB(pName, vName, AlleProjekte, Date.Now)
                    Dim existFit As Integer = Nothing
                    Dim existRisk As Integer = Nothing

                    If Not IsNothing(existingProj) Then
                        ' import as imp - variant Name
                        vName = "imp"
                        existFit = existingProj.StrategicFit
                        existRisk = existingProj.Risiko
                        If projDescription = "" Then
                            projDescription = existingProj.description
                        End If

                        If businessUnit = "" Then
                            businessUnit = existingProj.businessUnit
                        End If

                        If budget = 0 Then
                            budget = existingProj.Erloes
                        End If
                    ...
                    If pkDefinition.column.von > 0 Then
                        ...
                        Catch ex As Exception
                            pkDefinition.row.bis = pkDefinition.row.von - 1
                            outputline = "no Personalkosten found ... continued  "
                            Call logger(ptErrLevel.logInfo, outputline, "readCalcTemplatesWithConfig", anzFehler)
                        End Try
                    ...
                    ' read sk : SonstKosten
                    If skDefinition.column.von > 0 Then
                        ...
                        Catch ex As Exception
                            skDefinition.row.bis = skDefinition.row.von - 1
                            outputline = "no Sonst Kosten found ... continued  "
                            Call logger(ptErrLevel.logInfo, outputline, "readCalcTemplatesWithConfig", anzFehler)
                        End Try
                    ...
                    If zpDefinition.column.von > 0 Then
                        ...
                        Catch ex As Exception
                            zpDefinition.row.bis = zpDefinition.row.von - 1
                            outputline = "no Zahlungsplan found ... continued  "
                            Call logger(ptErrLevel.logInfo, outputline, "readCalcTemplatesWithConfig", anzFehler)
                        End Try
                    ...
                    ' now read the Orga-Unit Col, must not be empty 
                    ' must be defined, otherwise skip 
                    Dim orgaUnitCol As Integer
                    Try
                        If orgaUnitDefinition.column.von > 0 Then
                            orgaUnitCol = orgaUnitDefinition.column.von
                        Else
                            outputline = "No Definition in Config File where Orga-Unit can be found ... Exit "
                            Call logger(ptErrLevel.logError, outputline, "readCalcTemplatesWithConfig", anzFehler)
                        End If
                    Catch ex As Exception
                        Call logger(ptErrLevel.logError, ex.Message, "readCalcTemplatesWithConfig", anzFehler)
                        Throw New Exception(ex.Message)
                    End Try

                    ' now read the Skill col, may be empty or not even defined 
                    Dim skillCol As Integer = 0
                    Try
                        If skillDefinition.column.von > 0 Then
                            skillCol = skillDefinition.column.von
                        Else
                            outputline = "No Skills are to be considered  "
                            Call logger(ptErrLevel.logInfo, outputline, "readCalcTemplatesWithConfig", anzFehler)
                        End If
                    Catch ex As Exception
                        Call logger(ptErrLevel.logError, ex.Message, "readCalcTemplatesWithConfig", anzFehler)
                    ...
                    For zeile As Integer = pkDefinition.row.von To pkDefinition.row.bis
                        ...
                        ' read the phase-Name or define it by default as the project phase "." 
                        Try
                            If phaseDefinition.column.von > 0 Then
                                Dim phaseCol As Integer = phaseDefinition.column.von
                        ...
                        ' it is safe that orgaUnitCol does exist here ...
                        If Not IsNothing(currentWS.Cells(zeile, orgaUnitCol).value) Then
                            ...
                            If myOrgaUnit = "" Then
                                skipProcessingThisLine = True
                                outputline = "line not processed: " & zeile & " : contains empty Orga-Unit"
                                Call logger(ptErrLevel.logInfo, outputline, readCalcTemplatesWithConfig, anzFehler)
                            Else
                                If Not RoleDefinitions.containsName(myOrgaUnit) Then
                                    skipProcessingThisLine = True
                                    outputline = "line not processed: " & zeile & " : Orga-Unit not defined: " & myOrgaUnit
                                    Call logger(ptErrLevel.logInfo, outputline, readCalcTemplatesWithConfig, anzFehler)
                                End If
                            End If
                        Else
                        ...
                        ' now read the skill / role 
                        ' may be empty 
                        If skillCol > 0 Then
                            If Not IsNothing(currentWS.Cells(zeile, skillCol).value) Then
                                mySkill = CStr(currentWS.Cells(zeile, skillCol).value).Trim
                                If mySkill = "" Then
                                    ' is allowed ... 
                                    'Throw New Exception("Role must not be empty: Zeile" & zeile.ToString)
                                Else
                                    If Not RoleDefinitions.containsName(mySkill) Then
                                        ' maybe it is because there is only provided Senior or something like that instead of e.telligent - Senior 
                                        Dim possibleAlternatives As List(Of String) = RoleDefinitions.getSkillNamesContainingSubStr(mySkill, myOrgaUnit)
                                        If possibleAlternatives.Count <> 1 Then
                                            skipProcessingThisLine = True
                                            outputline = "line not processed: " & zeile & " : contains unknown skill: " & mySkill
                                            Call logger(ptErrLevel.logInfo, outputline, readCalcTemplatesWithConfig, anzFehler)
                                        Else
                                            mySkill = possibleAlternatives.First
                                        End If
                                    End If
                                End If
                            Else
                        ...
                        If Not skipProcessingThisLine Then

                            Dim myRoleNameID As String = RoleDefinitions.bestimmeRoleNameID(myOrgaUnit, mySkill)
                            If myRoleNameID = "" And Not skipProcessingThisLine Then
                                skipProcessingThisLine = True
                                outputline = "line not processed: " & zeile & "Invalid Combination" & myOrgaUnit & " , " & mySkill
                                Call logger(ptErrLevel.logInfo, outputline, readCalcTemplatesWithConfig, anzFehler)
                            End If
                            ...
                            For ix As Integer = 0 To arrayDimension
                                If Not IsNothing(currentWS.Cells(zeile, startIX + ix).value) Then
                                    ...
                                    If myValues(ix) < 0 Then
                                        outputline = "Negative resource needs are not allowed (row, column) " & zeile & " , " & ix
                                        Call logger(ptErrLevel.logError, outputline, readCalcTemplatesWithConfig, anzFehler)
                                        Throw New Exception(outputline)
                                    End If
                                Else
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    If roleValues.Item(myRoleNameID).Length <> myValues.Length Then
                                        Throw New Exception("Invalid Combination" & myOrgaUnit & " , " & mySkill)
                                    End If
                    ...
                    ' again , if not names of cost-.types are not existing: ignore line
                    ...
                    If skDefinition.row.von > 0 Then
                            ...
                            If Not IsNothing(currentWS.Cells(zeile, costCol).value) Then
                                ...
                                Else
                                    If Not CostDefinitions.containsName(myCost) Then
                                        skipProcessingThisLine = True
                                        outputline = "line not processed: " & zeile & " : contains unknown skill: " & myCost
                                        Call logger(ptErrLevel.logInfo, outputline, readCalcTemplatesWithConfig, anzFehler)
                                    End If
                                End If
                            Else
                        ...
                        For zeile As Integer = zpDefinition.row.von To zpDefinition.row.bis
                            ...
                            If Not IsNothing(currentWS.Cells(zeile, msCol).value) Then
                                ...
                                If myMilestoneName = "" Then
                                    ' do not nothing , just skip 
                                    skipProcessingThisLine = True
                                Else
                                    If milestoneNames.Contains(myMilestoneName) Then
                                        skipProcessingThisLine = True
                                        outputline = "line not processed: " & "Duplicate Milestone-Name: " & myMilestoneName
                                        Call logger(ptErrLevel.logInfo, outputline, readCalcTemplatesWithConfig, anzFehler)
                                    Else
                    ...
                    Dim newProj As clsProjekt = erstelleProjektausParametern(pName:=pName, vName:=vName, startDate:=startDate, endeDate:=endDate,
                                                                             budget:=budget, businessUnit:=businessUnit, description:=projDescription,
                                                                             responsible:=projektleiter, sfitKPI:=existFit, riskKPI:=existRisk,
                                                                             phaseDates:=phDates,
        ...
        Try
            If My.Computer.FileSystem.FileExists(tmpDatei) Then
                Try
                    ...
                    ' added 
                    Dim pTotalBudget As clsConfigProjectsImport = Nothing
                    Dim pProfit As clsConfigProjectsImport = Nothing
                    Try
                        pTotalBudget = projectConfig("Gesamt-Budget")
                    Catch ex As Exception

                    End Try

                    Try
                        pProfit = projectConfig("Profit")
                    Catch ex As Exception
                        ...
                        If phaseLength > 0 Then
                            ...
                            If phaseRoleValues.ContainsKey(phase.name) Then
                                ...
                                For Each role As KeyValuePair(Of String, Double) In roleList
                                    ...
                                    Dim roleDef As clsRollenDefinition = RoleDefinitions.getRoleDefByIDKennung(role.Key, skillID)
        ...
        Try
            If My.Computer.FileSystem.FileExists(JiraExcelFile) Then
                ...
                Try
                    ...
                    Dim vstart As New clsConfigProjectsImport
                    vstart = projectConfig("valueStart")
                    ' Auslesen erste Projekt-Spalte
                    firstUrlspalte = vstart.column.von
                    firstUrlzeile = vstart.row.von
